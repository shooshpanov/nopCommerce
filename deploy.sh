...
if [[ ! -n "$KUDU_SYNC_CMD" ]]; then
  ...
  sudo npm install kudusync -g --silent
...
dotnet restore "$DEPLOYMENT_SOURCE/src/NopCommerce.sln"
exitWithMessageOnError "dotnet restore failed"

# 2. Build and publish
dotnet publish "$DEPLOYMENT_SOURCE/src/Presentation/Nop.Web/Nop.Web.csproj" --output "$DEPLOYMENT_TEMP" --configuration Release
